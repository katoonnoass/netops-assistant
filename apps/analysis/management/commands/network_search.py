...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        sections = [
            ...
            ("VRF / L3VPN", results["vrf"]),
            ("QoS / Traffic Policy", results["qos"]),
            ("NAT / PAT", results["nat"]),
            ("IPv6 / BGP IPv6", results["ipv6"]),
            ("BNG / AAA / RADIUS / IP Pool", results["bng"]),
            ("HA / BFD / GR / NSR", results["ha"]),
            ("Multicast / PIM / IGMP / MLD", results.get("multicast", [])),
            ("PPPoE / Virtual-Template", results["pppoe"]),
            ("Huawei avançado", results.get("huawei_advanced", [])),
